...
import { matchItemText, matchItemLines, validateItemCatalog, phraseScore } from '../lib/items.js';
...
test('bundled item price catalogs are resolved and structurally valid', () => {
  for (const mode of ['pvp', 'pve', 'seasonal']) {
    const file = path.join(import.meta.dirname, '../app/data/items-' + mode + '.json'),
...
test('item OCR matching ranks exact and mildly noisy names first', () => {
  const catalog = JSON.parse(
    fs.readFileSync(path.join(import.meta.dirname, '../app/data/items-pvp.json'), 'utf8')
  );
