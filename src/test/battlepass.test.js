...
import { parseLiteral, parseMapData } from '../tools/build/build-battlepass.js';
...
test('Battle Pass locations preserve source positions and uncertainty across all 12 maps', () => {
  ...
  for (const map of doc.maps) {
    ...
    const source = parseMapData(
      fs.readFileSync(
        path.join(import.meta.dirname, '../tools/sources/battlepass/data-' + map.sourceKey + '.js'),
        'utf8'
      ),
      map.sourceKey
    );
...
test('English location notes are attached to real points and the Korean original is kept', () => {
  const translations = JSON.parse(
    fs.readFileSync(
      path.join(import.meta.dirname, '../tools/sources/battlepass/note-translations.json'),
      'utf8'
    )
  );
  const hangul = /[ᄀ-ᇿ㄰-㆏가-힯]/;
