...
test('Labs keycard legend resolves every colored-card door from bundled POIs', () => {
  const poi = JSON.parse(
    fs.readFileSync(path.join(__dirname, '../app/data/poi/the-lab.json'), 'utf8')
  );
  const catalog = JSON.parse(
    fs.readFileSync(path.join(__dirname, '../app/data/lab-keycards.json'), 'utf8')
  );
  ...
  for (const card of catalog.keycards) {
    ...
    const icon = path.join(__dirname, '../app/assets', card.iconPath.replace(/^\/+/, ''));
...
test('Reserve chess landmarks match the building clusters identified by RB keys', () => {
  ...
  const pois = JSON.parse(
      fs.readFileSync(path.join(__dirname, '../app/data/poi/reserve.json'), 'utf8')
    ).pois,
    anchors = {
...
test('complete quest catalogs are bundled and the UI does not force a Customs-only list', () => {
  for (const [file, minimum] of [
    ...
    const catalog = JSON.parse(
      fs.readFileSync(path.join(__dirname, '../app/data', file), 'utf8')
    ).quests;
    ...
    const maps = new Set(
      JSON.parse(fs.readFileSync(path.join(__dirname, '../app/data/maps.json'), 'utf8')).map(
        map => map.id
      )
    );
    assert.ok(
  ...
  assert.deepEqual(
    sheets,
    ['app.css', 'battlepass.css'],
    'the renderer loads the merged stylesheet and the Battle Pass layer, in that order'
  );
  for (const sheet of sheets)
    assert.ok(fs.existsSync(path.join(__dirname, '../app', sheet)), sheet + ' is bundled');
  ...
  for (const retired of [
    ...
    assert.ok(
      !fs.existsSync(path.join(__dirname, '../app', retired)),
      retired + ' was merged into app.css and must not come back'
    );
...
test('every declared map has bundled artwork, POIs and a selectable location', () => {
  ...
  for (const map of maps) {
    ...
    for (const asset of assets)
      assert.ok(
        fs.existsSync(path.join(__dirname, '../app/assets', asset.path.replace(/^\/+/, ''))),
        `${map.id}: ${asset.path}`
      );
    assert.ok(
      fs.existsSync(path.join(__dirname, '../app/data/poi', map.id + '.json')),
      `${map.id}: POI bundle`
    );
  }
  const html = fs.readFileSync(path.join(__dirname, '../app/index.html'), 'utf8'),
    renderer = fs.readFileSync(path.join(__dirname, '../app/app.js'), 'utf8');
...
test('every declared map has a validated loot layer and every marker icon is bundled', () => {
  ...
  let total = 0;
  for (const map of maps) {
    const file = path.join(__dirname, '../app/data/loot', map.id + '.json');
    ...
    for (const [x, y, z, type] of loot.containers || []) {
      ...
      checkedIcons.add(path.join(__dirname, '../app/assets/loot/containers', meta.icon));
    }
    for (const [x, y, z, ids, categories] of loot.loose || []) {
      ...
      for (const id of ids) {
        ...
        checkedIcons.add(path.join(__dirname, '../app/assets/loot/items', id + '.webp'));
      }
    }
    for (const meta of Object.values(loot.categories))
      checkedIcons.add(path.join(__dirname, '../app/assets/loot/categories', meta.icon));
  }
...
test('story chapters and Battle Pass documents are bundled as separate tracks', () => {
  const data = JSON.parse(
    fs.readFileSync(path.join(__dirname, '../app/data/special-tracks.json'), 'utf8')
  );
...
test('every locked door resolves to a key name the map can print', () => {
  ...
  const keys = JSON.parse(
    fs.readFileSync(path.join(__dirname, '../app/data/keys.json'), 'utf8')
  ).keys;
  const cards = new Set(
    JSON.parse(
      fs.readFileSync(path.join(__dirname, '../app/data/lab-keycards.json'), 'utf8')
...
test('keys are shown with the image the game uses', () => {
  ...
  const onDisk = fs
    .readdirSync(path.join(__dirname, '../app/assets/keys'))
...
test('quest pictures and boss portraits are bundled and wired', () => {
  ...
  const images = JSON.parse(
    fs.readFileSync(path.join(__dirname, '../app/data/quest-images.json'), 'utf8')
  ).images;
  const bosses = JSON.parse(
    fs.readFileSync(path.join(__dirname, '../app/data/bosses.json'), 'utf8')
  ).bosses;
  const magic = file => {
    const bytes = fs.readFileSync(file);
  ...
  for (const file of fs.readdirSync(path.join(__dirname, '../app/data/poi')))
    for (const poi of JSON.parse(
      fs.readFileSync(path.join(__dirname, '../app/data/poi', file), 'utf8')
    ).pois)
...
test('boss spawn chances follow the profile in play', () => {
  ...
  const doc = JSON.parse(
    fs.readFileSync(path.join(__dirname, '../app/data/boss-spawns.json'), 'utf8')
  );
  for (const mode of ['pvp', 'pve', 'seasonal']) assert.ok(doc.modes[mode], 'no rates for ' + mode);
  const maps = JSON.parse(
    fs.readFileSync(path.join(__dirname, '../app/data/maps.json'), 'utf8')
  ).map(map => map.id);
...
test('quest events name their trader so unlisted quests can still be identified', () => {
  ...
  const traders = JSON.parse(
    fs.readFileSync(path.join(__dirname, '../app/data/traders.json'), 'utf8')
  ).traders;
  assert.ok(
...
test('every module the main process loads is one the installed copy gets', () => {
  ...
  for (const source of [
    ...
    for (const found of read(source).matchAll(/require\('\.\/([\w.-]+\.cjs)'\)/g))
      required.add(found[1]);
  assert.ok(required.size >= 5, 'expected the main process to load several local modules');
  const sync = read('tools/release/sync-installed.cjs'),
    pack = read('tools/release/package.cjs');
  for (const module of required) {
    assert.ok(
      sync.includes("'" + module + "'"),
      module + ' is loaded at runtime but tools/release/sync-installed.cjs does not copy it'
    );
    assert.ok(
      pack.includes("'" + module + "'"),
      module + ' is loaded at runtime but tools/release/package.cjs does not package it'
    );
  }
});
