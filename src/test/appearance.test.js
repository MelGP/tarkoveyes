...
test('colour signatures separate the things the reading confuses', () => {
  const a = signatureOf(block(16, 16, red), 16, 16),
    b = signatureOf(block(16, 16, green), 16, 16);
...
test('the scan takes its colour sample before the text is lifted out of it', () => {
  // The cursor scan moved out of main.js into item-hotkey.js. Reading the
  // wrong file here would not have failed loudly - assert.match on a file that
  // no longer contains the code is a failure, but assert.doesNotMatch on it
  // passes for the wrong reason, and half of this test is a doesNotMatch.
  const main = fs.readFileSync(path.join(import.meta.dirname, '../main/item-hotkey.js'), 'utf8');
...
test('every item the price catalogs list has a picture on file', () => {
  const file = path.join(import.meta.dirname, '../app/data/item-appearance.json'),
  ...
  const catalog = JSON.parse(
    fs.readFileSync(path.join(import.meta.dirname, '../app/data/items-seasonal.json'), 'utf8')
  );
  const covered = catalog.items.filter(item => doc.signatures[item.id]).length;
  assert.ok(
    covered / catalog.items.length > 0.85,
    'only ' + covered + ' of ' + catalog.items.length + ' covered'
  );
