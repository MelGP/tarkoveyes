...
function phraseScore(expected, actual) {
  ...
  const aw = a.split(' ').filter(Boolean),
    bw = new Set(b.split(' ').filter(Boolean)),
    coverage = aw.filter(word => bw.has(word)).length / aw.length;
  return coverage * 0.62 + dice(a, b) * 0.38;
}
function matchItemText(text, catalog, limit = 6) {
  ...
  for (const item of catalog.items) {
    ...
    for (const phrase of phrases) {
      let current = phraseScore(item.name, phrase);
      const short = normalizeText(item.shortName);
      if (short.length >= 4)
        current = Math.max(current, phraseScore(item.shortName, phrase) * 0.94);
      else if (
        short.length >= 3 &&
        new RegExp('(?:^| )' + short.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(?: |$)').test(
    ...
    if (fullName.length >= 5 && all.includes(fullName)) score = Math.max(score, 0.99);
    if (score >= 0.43)
      results.push({ ...item, confidence: Number(score.toFixed(2)), matchedText: matched });
  }
  return results
    .sort(
      (a, b) =>
