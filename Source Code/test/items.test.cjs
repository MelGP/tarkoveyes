...
test('the item hotkey picks the row under the cursor, not its neighbours',()=>{
  const catalog=JSON.parse(fs.readFileSync(path.join(__dirname,'../app/data/items-pvp.json'),'utf8'));
  const stack=['AFAK tactical individual first ai','AI-2 medkit (22)','Car first aid kit (19)','Grizzly medical kit (25)','IFAK individual first aid kit (36)'];
  ...
  for(const [on,expected,neighbour] of [[1,'AI-2 medkit','Car first aid kit'],[2,'Car first aid kit','Grizzly medical kit'],[3,'Grizzly medical kit','IFAK individual first aid kit']]){
    ...
    if(other){
      ...
      assert.ok(other.score<found[0].score*.5);
    }
  }
  // a neighbouring row must not borrow the cursor's own position through the
  // joined pair that spans them both
  const caps=matchItemLines(rows(['Bandana (1)','Baseball cap (3)','Baseball cap (Boston) (1)'],1),catalog,3);
...
test('names lost to look-alike characters are still found',()=>{
  ...
  assert.equal(matchItemLines(rows(['Survi2'],0),catalog,3)[0].name,'Surv12 field surgical kit');
...
test('a wrapped item name still matches, and confidence stays the text match',()=>{
  const catalog=JSON.parse(fs.readFileSync(path.join(__dirname,'../app/data/items-pvp.json'),'utf8'));
  const wrapped=matchItemLines(rows(['Salewa first aid','kit (12)','Weight 0.4 kg'],0),catalog,3);
  assert.equal(wrapped[0].name,'Salewa first aid kit');
  // distance fades the ranking score but never the reported read quality
  const far=matchItemLines([{text:'Graphics card',left:180,right:290,top:120,bottom:134}],catalog,3);
  assert.equal(far[0].name,'Graphics card');
  assert.equal(far[0].confidence,1);
  assert.ok(far[0].score<.2);
  assert.deepEqual(matchItemLines([],catalog,3),[]);
  assert.deepEqual(matchItemLines([{text:'  ',left:0,right:9,top:0,bottom:9}],catalog,3),[]);
});
