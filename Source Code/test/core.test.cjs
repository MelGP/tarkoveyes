...
test('every module the main process loads is one the installed copy gets', () => {
  ...
  const sync = read('tools/sync-installed.cjs'),
    pack = read('tools/package.cjs');
  for (const module of required) {
    assert.ok(
      sync.includes("'" + module + "'"),
      module + ' is loaded at runtime but tools/sync-installed.cjs does not copy it'
    );
    assert.ok(
      pack.includes("'" + module + "'"),
      module + ' is loaded at runtime but tools/package.cjs does not package it'
    );
  }
});
