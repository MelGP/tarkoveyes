...
$incoming = [IO.Path]::GetFullPath((Join-Path $targetRoot '.incoming-v0.18.1'))
$backup = [IO.Path]::GetFullPath((Join-Path $targetRoot ('Backups\RaidNotes-v0.18.0-before-v0.18.1-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))))
...
if ($packagedManifest.version -ne '0.18.1') { throw "Unexpected packaged version: $($packagedManifest.version)" }
...
if ($incomingManifest.version -ne '0.18.1' -or $sourceManifest.version -ne '0.18.1') { throw 'Unable to validate the staged release.' }
...
if ($installedManifest.version -ne '0.18.1') { throw 'Installed version validation failed.' }
$process = Start-Process -FilePath (Join-Path $targetRoot 'App\RaidNotes.exe') -WorkingDirectory (Join-Path $targetRoot 'App') -PassThru
