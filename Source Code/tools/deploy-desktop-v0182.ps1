...
$incoming = [IO.Path]::GetFullPath((Join-Path $targetRoot '.incoming-v0.18.2'))
$backup = [IO.Path]::GetFullPath((Join-Path $targetRoot ('Backups\RaidNotes-v0.18.1-before-v0.18.2-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))))
...
if ($packagedManifest.version -ne '0.18.2') { throw "Unexpected packaged version: $($packagedManifest.version)" }
...
if ($incomingManifest.version -ne '0.18.2' -or $sourceManifest.version -ne '0.18.2') { throw 'Unable to validate the staged release.' }
...
if ($installedManifest.version -ne '0.18.2') { throw 'Installed version validation failed.' }
