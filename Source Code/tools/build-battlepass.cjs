...
const upstream = 'https://raw.githubusercontent.com/Perofunyang/battlepass_interactive_map/' + commit + '/';
...
function parseLiteral(text) {
  ...
  function skip() {
    for (;;) {
      ...
      else if (text.slice(i, i + 2) === '/*') { const end = text.indexOf('*/', i + 2); if (end < 0) throw Error('Unclosed comment'); i = end + 2; }
      else break;
    }
  }
  function string() {
    ...
    while (i < text.length) {
      ...
      if (quote === '`' && char === '$' && text[i] === '{') throw Error('Template expressions are forbidden');
      if (char === '\\') { const next = text[i++]; const escapes = { n:'\n', r:'\r', t:'\t', b:'\b', f:'\f' }; value += escapes[next] ?? next; }
      else value += char;
    }
    throw Error('Unclosed string');
  }
  function value() {
    skip(); const char = text[i];
    if ('"\'`'.includes(char) && char) return string();
    if (char === '[' || char === '{') {
      i++; const array = char === '[', result = array ? [] : Object.create(null), close = array ? ']' : '}'; skip();
      while (text[i] !== close) {
        if (array) result.push(value());
        else { skip(); if (!'"\''.includes(text[i])) throw Error('Expected a quoted key'); const key = string(); if (['__proto__','constructor','prototype'].includes(key)) throw Error('Unsafe key'); skip(); if (text[i++] !== ':') throw Error('Expected colon'); result[key] = value(); }
        skip(); if (text[i] === close) break; if (text[i++] !== ',') throw Error('Expected comma at ' + i); skip();
      }
      i++; return result;
    }
    const match = text.slice(i).match(/^(?:-?\d+(?:\.\d+)?|true|false|null)\b/);
    if (!match) throw Error('Non-data expression at ' + i);
    i += match[0].length; return JSON.parse(match[0]);
  }
  const result = value(); skip(); if (text[i] === ';') i++; skip(); if (i !== text.length) throw Error('Unexpected trailing expression'); return result;
}
...
const documentIds = {
  blueprints:'6a31824878450ec91c0ea1ae', financial:'6a31807f17005505b70d5827',
  medical:'6a3182dc6cd8de21cf0a3a7d', pmc:'6a317b9692cfdcddcb02a58e',
  project:'6a3181f178450ec91c0ea1aa', technical:'6a31830dde69ceafd805afa0',
  test:'6a31828557705071410ca00e', user:'6a3182b72fd891345e047eef',
...
function build() {
  ...
  const categories = Object.fromEntries(Object.entries(documentIds).map(([key,id]) => {
    if (!items[id]?.handbookCategories.includes('6a35427afc3f27b15905a876')) throw Error('Unverified Battle Pass document: ' + id);
    return [key, {id, name:english[items[id].name], label:labels[key], color:colors[key], icon:'assets/battlepass/icons/' + id + '.webp'}];
  }));
  const maps = [], downloads = new Map();
  for (const match of sourceApp.matchAll(/^\s*(\w+): \{ name: "([^"]+)", imageUrl: "\.\/(assets\/maps\/[^\"]+)", width: (\d+), height: (\d+)/gm)) {
    const [,key,sourceName,imagePath,width,height] = match, id = aliases[key] || key;
    const w = Number(width), h = Number(height), rows = parseMapData(fs.readFileSync(path.join(sourceDir,'data-' + key + '.js'),'utf8'), key);
    ...
    for (const row of rows) {
      ...
      if (!Array.isArray(row.coords) || row.coords.length !== 2 || !row.coords.every(Number.isFinite)) throw Error('Invalid coordinates: ' + row.id);
      const [sourceY,x] = row.coords, y = h - sourceY;
      if (x < 0 || x > w || y < 0 || y > h) throw Error('Point outside source image: ' + key + '/' + row.id);
      if (seen.has(row.id)) throw Error('Duplicate point ID: ' + key + '/' + row.id); seen.add(row.id);
      const photos = [...new Set([row.previewImg,...(Array.isArray(row.detailImg) ? row.detailImg : [row.detailImg])].filter(Boolean))];
    ...
    maps.push({id,sourceKey:key,name:id==='the-lab'?'The Lab':id==='the-labyrinth'?'The Labyrinth':sourceName,width:w,height:h,image,points});
  }
  if (maps.length !== 12) throw Error('Expected 12 community maps');
  for (const category of Object.values(categories)) downloads.set(category.icon,'https://assets.tarkov.dev/' + category.id + '-icon.webp');
  const devSpawnCount = Object.values(devMaps).reduce((count,map)=>count+(map.lootLoose||[]).filter(point=>point.items.some(id=>Object.values(documentIds).includes(id))).length,0);
...
async function downloadAssets(downloads) {
  const jobs=[...downloads];let cursor=0;
  async function worker(){while(cursor<jobs.length){const [relative,url]=jobs[cursor++],target=path.join(root,'app',relative);if(fs.existsSync(target)&&fs.statSync(target).size>100)continue;const response=await fetch(url);if(!response.ok)throw Error(response.status+' '+url);const bytes=Buffer.from(await response.arrayBuffer());if(bytes.length<100)throw Error('Empty asset: '+url);fs.mkdirSync(path.dirname(target),{recursive:true});fs.writeFileSync(target,bytes);}}
  await Promise.all(Array.from({length:6},worker));console.log('All reference maps, document icons and spawn photographs are bundled.');
}
module.exports={parseLiteral,parseMapData};
if(require.main===module){const downloads=build();if(process.argv.includes('--download-assets'))downloadAssets(downloads).catch(error=>{console.error(error);process.exitCode=1;});}
