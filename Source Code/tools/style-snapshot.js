...
(async () => {
  ...
  async function setFilters(map, status) {
    const m = document.getElementById('map-filter'),
      s = document.getElementById('status-filter');
    m.value = map;
  ...
  const scenes = {
    async main() {
      ...
      for (let i = 0; i < 60 && !document.getElementById('map-loading').hidden; i++)
        await wait(250);
    ...
    async items() {
      closeDialogs();
      document.getElementById('items-button').click();
      await wait(800);
    },
    async connection() {
      closeDialogs();
    ...
    async command() {
      closeDialogs();
      document.getElementById('quick-find').click();
      await wait(600);
    }
  };
  ...
  if (!baseline) {
    ...
    const counts = Object.fromEntries(
      Object.entries(taken).map(([k, v]) => [k, Object.keys(v).length])
    );
  ...
  for (const name of Object.keys(scenes)) {
    ...
    for (const k of new Set([...Object.keys(before), ...Object.keys(after)])) {
      ...
      if (!(k in before)) {
        diffs.push({ scene: name, el: k, change: 'added' });
        continue;
      }
      if (!(k in after)) {
        diffs.push({ scene: name, el: k, change: 'removed' });
        continue;
      }
      ...
      diffs.push({
        scene: name,
        el: k,
        change: PROPS.map((p, i) => (b[i] === a[i] ? null : p + ': ' + b[i] + ' -> ' + a[i]))
          .filter(Boolean)
          .join('; ')
      });
    }
