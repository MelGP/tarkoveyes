...
function metadata(item) {
  ...
  for (const id of [...ids]) {
    let current = raw.handbookCategories[id];
    while (current?.parent && !ids.has(current.parent)) {
      ids.add(current.parent);
      current = raw.handbookCategories[current.parent];
    }
  }
  return {
    category: definitions.find(([, id]) => ids.has(id))?.[0] || 'Other',
    handbookCategories: [...(item.handbookCategories || [])]
  };
}
for (const mode of ['pvp', 'pve', 'seasonal']) {
  const target = path.join(root, 'app/data/items-' + mode + '.json'),
    doc = JSON.parse(fs.readFileSync(target)),
    byId = new Map(doc.items.map(item => [item.id, item]));
  for (const item of Object.values(raw.items)) {
    ...
    if (existing) Object.assign(existing, meta);
      ...
      byId.set(item.id, {
        id: item.id,
        name: english[item.name] || item.normalizedName,
        shortName: english[item.shortName] || english[item.name] || item.normalizedName,
        normalizedName: item.normalizedName || '',
        updated: item.updated || null,
        width: Math.max(1, Number(item.width) || 1),
        height: Math.max(1, Number(item.height) || 1),
        avg24hPrice: Number.isFinite(item.avg24hPrice) ? item.avg24hPrice : null,
        low24hPrice: Number.isFinite(item.low24hPrice) ? item.low24hPrice : null,
        high24hPrice: Number.isFinite(item.high24hPrice) ? item.high24hPrice : null,
        lastLowPrice: Number.isFinite(item.lastLowPrice) ? item.lastLowPrice : null,
        changeLast48hPercent: Number.isFinite(item.changeLast48hPercent)
          ? item.changeLast48hPercent
          : null,
        minLevelForFlea: Number.isFinite(item.minLevelForFlea) ? item.minLevelForFlea : null,
  ...
  doc.items = [...byId.values()].sort((a, b) => a.name.localeCompare(b.name));
  fs.writeFileSync(target, JSON.stringify(doc));
  console.log(mode + ': ' + doc.items.length + ' categorized items');
}
