...
$incoming = [IO.Path]::GetFullPath((Join-Path $targetRoot '.incoming-v0.16.0'))
$backup = [IO.Path]::GetFullPath((Join-Path $targetRoot ('Backups\RaidNotes-v0.15.0-before-v0.16.0-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))))
...
if ($packagedManifest.version -ne '0.16.0') { throw "Unexpected packaged version: $($packagedManifest.version)" }
...
if ($incomingManifest.version -ne '0.16.0' -or $sourceManifest.version -ne '0.16.0') { throw 'Unable to validate the staged release.' }
...
if ($installedManifest.version -ne '0.16.0') { throw 'Installed version validation failed.' }
