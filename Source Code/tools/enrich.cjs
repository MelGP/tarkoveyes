...
function itemName(names,id){return names[id+' Name']||names[id+' name']||id;}
// tarkov.dev keeps separate ids for map variants; Raid Notes bundles one map for each.
const mapAliases={'night-factory':'factory','ground-zero-21':'ground-zero','ground-zero-tutorial':'ground-zero','the-lab-dark':'the-lab'};
async function mapIdsByTarkovDevId(){
  ...
  const bundled=new Set(JSON.parse(fs.readFileSync('app/data/maps.json','utf8')).map(map=>map.id));
  const byId=new Map();
...
async function run(){
  ...
  for(const [mode,file] of [['regular','quests'],['pve','quests-pve']]){
    ...
    for(const q of doc.quests){
      ...
      q.neededKeys=[...keys.values()].map(entry=>entry.mapIds.length?entry:{id:entry.id,name:entry.name});
      for(const o of q.objectives){const item=task.objectives.find(x=>x.id===o.id);if(!item)continue;
        o.requiredKeys=(item.requiredKeys||[]).map(group=>group.map(id=>itemName(names,id)));
        o.itemNames=(item.items||[]).slice(0,12).map(id=>itemName(names,id));
      }
    }
    const withKeys=doc.quests.filter(q=>q.neededKeys?.length).length;
    const mapsCovered=new Set(doc.quests.flatMap(q=>(q.neededKeys||[]).flatMap(k=>k.mapIds||[])));
    if(!dry){
      doc.requirementsUpdatedAt=new Date().toISOString();
      fs.writeFileSync('app/data/'+file+'.json',JSON.stringify(doc));
    }
    console.log(mode+': '+withKeys+' quests carry keys across '+mapsCovered.size+' bundled maps'
      +(missingMaps?' ('+missingMaps+' key groups on maps this build does not bundle)':'')
