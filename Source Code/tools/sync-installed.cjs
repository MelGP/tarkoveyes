...
const runtimeFiles=['core.cjs','items.cjs','main.cjs','preload.cjs','LICENSE','THIRD-PARTY.md'];
...
function readManifest(){
  ...
  return [...raw.matchAll(/^\s*"([^"]+)":\s*"([0-9a-f]{64})",?\s*$/gm)].map(match=>[match[1],match[2]]);
}
...
for(const relative of runtimePaths()){
  const from=path.join(source,relative),to=path.join(installed,relative);
  if(!fs.existsSync(to)){added.push(relative);continue;}
  if(hash(from)!==hash(to))changed.push(relative);
}
for(const relative of [...runtimeFiles,...runtimeDirs.flatMap(dir=>walk(path.join(installed,dir),installed))])
  if(fs.existsSync(path.join(installed,relative))&&!fs.existsSync(path.join(source,relative)))removed.push(relative);

const report=[['changed',changed],['new in source',added],['missing from source',removed]];
for(const [label,list] of report){
  if(!list.length)continue;
  console.log(label+': '+list.length);
  for(const item of list)console.log('  '+item);
}
if(!changed.length&&!added.length&&!removed.length)console.log('Installed copy already matches Source Code.');
if(check||(!changed.length&&!added.length&&!removed.length))process.exit(0);
...
for(const relative of [...changed,...added])byKey.set('resources/app/'+relative,hash(path.join(installed,relative)));
...
console.log('\nSynced '+(changed.length+added.length)+' file(s), removed '+removed.length+'.');
