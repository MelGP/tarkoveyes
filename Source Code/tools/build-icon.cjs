 ...
 * --remote-debugging-port=9222, the same way tools/renderer-smoke.js is driven,
 * and for the same reason tools/build-appearance.cjs runs under Electron - that
...
(async () => {
  ...
  for (const i of images)
    console.log('  ' + String(i.size).padStart(3) + 'px  ' + i.kind + '  ' + i.body.length + ' bytes');
  console.log(
