...
contextBridge.exposeInMainWorld('companion',{
  ...
  onObserver:callback=>{const handler=(_e,state)=>callback(state);ipcRenderer.on('observer',handler);return()=>ipcRenderer.removeListener('observer',handler);}
  ,onQuestProgress:callback=>{const handler=(_e,event)=>callback(event);ipcRenderer.on('quest-progress',handler);return()=>ipcRenderer.removeListener('quest-progress',handler);}
  ,onOcrProgress:callback=>{const handler=(_e,event)=>callback(event);ipcRenderer.on('ocr-progress',handler);return()=>ipcRenderer.removeListener('ocr-progress',handler);}
  ,onItemHotkey:callback=>{const handler=(_e,event)=>callback(event);ipcRenderer.on('item-hotkey-status',handler);return()=>ipcRenderer.removeListener('item-hotkey-status',handler);}
  ,onItemHotkeyResult:callback=>{const handler=(_e,event)=>callback(event);ipcRenderer.on('item-hotkey-result',handler);return()=>ipcRenderer.removeListener('item-hotkey-result',handler);}
});
